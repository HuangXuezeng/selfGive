        ...
        <van-row type="flex">
            <van-col span="24">
                <div class="resetVantArewards">
                    <van-field readonly clickable label="职级:" label-class="labelStyle" v-model="selectzjName" placeholder="请选择职级" @click="rightPopShow" />
                </div>
                <van-popup v-model="rightPop" position="right" :overlay="true" :lazy-render="true" style="width: 50%; height: 100%;" :get-container="getContainer">
                    <div style="height: 90%;overflow: auto;z-index: 2049;">
                        <div v-for="item in checkboxlist" :key="item.zl">
                            <van-cell clickable @click="toggleZL(item)">
                                ...
                                <template #right-icon>
                                    <van-checkbox :name="item.zlNum" v-model="item.zlchecked" ref="checkboxesZL" shape="square" />
                                </template>
                            </van-cell>
                            <van-checkbox-group v-model="result">
                                <van-cell-group>
                                    <van-cell v-for="items in item.zjList" clickable :key="items.number" :title="`${items.zj}`" @click="toggle(items)">
                                        <template #right-icon>
                                            <van-checkbox :name="items.indexNumber" ref="checkboxes" />
                                        </template>
                                    </van-cell>
                                </van-cell-group>
                            </van-checkbox-group>
                    ...
                    <van-row type="flex" justify="space-around">
                        <van-col span="10">
                            <van-button type="info" size="normal" @click="restSelectZj">重置</van-button>
                        </van-col>
                        <van-col span="10">
                            <van-button :loading="selctzjqueryLoading" loading-type="spinner" type="primary" size="normal" @click="selctzjquery">确认</van-button>
                        </van-col>
                    </van-row>
                </van-popup>
            </van-col>
