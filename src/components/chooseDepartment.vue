    ...
    <div class="resetVant">
        ...
        <van-popup v-model="showPickDept" position="top" :style="{ height: '70%' }" get-container="body" :closeable="closeableFlag">
            <div style="padding-bottom:12%">
                ...
                <van-cell center title="是否自动包含下级部门" title-style="color:red" icon="setting" v-if="isSelctall">
                    <template #right-icon>
                        <van-switch :value="isDownValue" @input="isDownMethods" size="15px" />
                    </template>
                </van-cell>
                <el-tree :data="deptData" ref="tree" node-key="deptId" @check-change="handleCheckChange" @check="judgeLowerLevel" :props="props" show-checkbox :check-strictly="strictlyFlag" :default-expanded-keys="openlist"></el-tree>
            </div>
            <div class="butnPosition" v-if="isSelctall">
                <div class="butnPos">
                    <div class="buttonChoose" style="background-color:MediumAquamarine" @click="RemoveNodeMeth">
                        重置
                    </div>
                    <div class="buttonChoose" :style="selectAllstyle" @click="selctAllNodeMeth">
                        全选
                    </div>
                    <div class="buttonChoose" style="background-color:DeepSkyBlue" @click="confirmNodeMeth">
                        确认
                    </div>
                </div>
            </div>
    ...
    export default {
        ...
        mounted() {
            if (this.isSelctall) {
                ...
                } else if (this.isCareerPath) {
                    let readySelectObj = JSON.parse(localStorage.getItem("adresResultDept"))
                    this.selectedDepartment = readySelectObj.text
                    this.beforeSelectName = readySelectObj.text
                } else {
        ...
        methods: {
            ...
            pickDept() {
                ...
                this.$nextTick(() => {
                    if (this.isCareerPath) {
                        this.deptData = this.faDeptData;
                        this.openlist = this.openKeyList;
                    } else {
