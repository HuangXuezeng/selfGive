    ...
    <div style="padding-bottom: 10%;">
        ...
        <div>
            ...
            <div>
              ...
              force-ellipses
            /> -->
                <pagination :Fatotal="total" :Faseachflag="seachFlag" @queryNewSearch="upDateNewSearch" @queryNew="queryFindPayrollInfo" ref="Paypagination"></pagination>
                <van-tabs v-model="active" type="card" class="resetVant" animated>
                    <van-tab name="简略">
                        <template #title>
                            <van-icon name="coupon-o" size="20px" />简略
                        </template>
                        <v-table is-horizontal-resize style="width:100%;" is-vertical-resize :columns="briefColumns" :table-data="tableConfig.tableData" row-hover-color="#eee" row-click-color="#edf7ff" :is-loading="isLoading" :paging-index="(pageIndex - 1) * pageSize" :vertical-resize-offset="tableBootomHight" :even-bg-color="'LightGrey'"></v-table>
                    </van-tab>
                    <van-tab name="详情">
                        <template #title>
                        ...
                        <div>
                            <v-table is-horizontal-resize style="width:100%;" is-vertical-resize :min-height="300" :title-rows="tableConfig.titleRows" :columns="tableConfig.columns" :table-data="tableConfig.tableData" row-hover-color="#eee" row-click-color="#edf7ff" :is-loading="isLoading" :paging-index="(pageIndex - 1) * pageSize" :vertical-resize-offset="tableBootomHight" :even-bg-color="'LightGrey'"></v-table>
                        </div>
                    </van-tab>
                </van-tabs>
          ...
          <template #default>
            <van-stepper v-model="searchObj.num" />
                        ...
                        <van-row type="flex" justify="space-around" style="margin-top:20px">
                            <van-col span="8">
                                <van-button type="primary" size="normal" @click="restSeach" style="width:15vh">重置</van-button>
                            </van-col>
                            <van-col span="8">
                                <van-button type="danger" size="normal" @click="confirmSearch" style="width:15vh">确定</van-button>
                            </van-col>
                        </van-row>
                    </div>
                </van-popup>
            ...
            <div>
                ...
                <div>
                    <choosedepartment @confirmNode="tantileSelectdept" :Farequired="true" labelTitle="部门:" :workingNum="true" :isSelctall="true" :faDeptData="deptData" ref="salaryDep"></choosedepartment>
                    <chooseZJlist ref="chooseZJlistRef" :checkboxlist="checkboxlist" :selectZjNameMap="selectZjNameMap" @confirmZj="confirmZjMethod"></chooseZJlist>
                    <div style="width:100%;" v-if="!showNodata">
                        <div ref="tantileEchart" :style="{ width: '100%', height: '500px' }"></div>
                    </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.$refs.payTab.changtab('SalaryLibrary')
        },
        methods: {
            initMethods() {
